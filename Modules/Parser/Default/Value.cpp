...
ast::Expression astClass::GetBaseTypeDefaultValue(const sPtr<type::BaseType> &_type) const {
    return GetBaseTypeDefaultValue(*_type);
}

ast::Expression astClass::GetBaseTypeDefaultValue(const type::BaseType &_type) const {
...
ast::Expression astClass::GetStructDefaultValue(const sPtr<type::StructDefinition> &_type) {
    return GetStructDefaultValue(*_type);
}

ast::Expression mlc::parser::AbstractSyntaxTree::GetStructDefaultValue(const type::StructDefinition &_type) {
    auto memberInits = _type.Members
                       | std::views::transform([this](const type::StructMember &member) {
                           return ast::MakeExpression(GetDefaultValue(member.Type));
    ...
    return std::visit(
        overloaded{
            [&](const type::BaseType &type) {
                return GetBaseTypeDefaultValue(type);
            },
            [&](const type::EnumDefinition &) {
                return ast::Expression(ast::ConstValue("0"));
            },
            [&](const type::StructDefinition &type) {
                return GetStructDefaultValue(type);
            },
            [&](const type::ArrayType &type) {
                return ast::Expression(ast::MakeInitializerList(std::vector(
                    type.Length, ast::MakeExpression(GetDefaultValue(type.BaseType)))));
            },
...
ast::Expression astClass::FillDefaultValue(const sPtr<type::CompileType> &_type,
    ...
    if (_initExpr == nullptr) {
        if (const auto *const arrayPtr = type::GetType<type::ArrayType>(_type)) {
            auto defaultValues = std::views::iota(0ULL, arrayPtr->Length)
                                 | std::views::transform([&](auto) {
                                     return ast::MakeExpression(GetDefaultValue(arrayPtr->BaseType));
        ...
        if (const auto *const structPtr = type::GetType<type::StructDefinition>(_type)) {
    ...
    if (const auto *const list = std::get_if<sPtr<ast::InitializerList> >(&*_initExpr->Storage)) {
        size_t expectedSize = 0;
        sPtr<type::CompileType> typeToFill;
        const auto *const arrayPtr = type::GetType<type::ArrayType>(_type);
        const auto *const structPtr = type::GetType<type::StructDefinition>(_type);
        if (arrayPtr) {
